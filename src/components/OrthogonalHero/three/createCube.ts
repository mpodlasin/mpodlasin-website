...
export default function createCubeFactory({
  ...
  async function createCube() {
    ...
    const crossing = -3;
    const bodiesWithIndexes = bodies.map(
      (body, index) => [body, index] as const,
    );
    const fallenBodies = bodiesWithIndexes.filter(
      ([body]) => body.translation().y < crossing,
    );
    const survivedBodies = bodiesWithIndexes.filter(
      ([body]) => body.translation().y >= crossing,
    );
    ...
    colliders = colliders.filter((_, index) =>
      survivedBodiesIndexes.includes(index),
    );
